...
function modalPop(){
	...
	$('.js_pop_close, #btn_link_academy, #dim').on('click', function(){
...
function interviewSwiper(){
	let intvSwiper = new Swiper('.js_interview_swiper',{
		slidesPerView: '1',
		spaceBetween : 20,
		loop:true,
...
// secret code swiper
function scSwiper(){
	let scSwiper = new Swiper('.js_sc_swiper',{
		slidesPerView: '1',
		spaceBetween : 20,
		loop:true,
		loopAdditionalSlides: 1,
		...
		navigation: {
			prevEl: '.sc_arrow .arr_prev',
			nextEl: '.sc_arrow .arr_next',
		},
		pagination: {
			el: '.sc_paging.swiper_bullet',
			type: 'bullets',
			clickable: true,
		},
	});
...
function youtubePop(){
	...
	$('.js_btn_vod_modal').on('click', function(){
		...
		const frameStart = "<iframe src='https://youtube.com/embed/";
...
function parentFormScroll(){
	setTimeout(function(){
		...
		let targetSec = document.getElementById('quick_sec_form');
		let targetPoint = targetSec.getBoundingClientRect().top + window.scrollY;
...
function quickForm(){
  ...
  // 240701 미사용 주석처리
	...
	// 	console.log(crtTop);
	// 	if(targetPoint <= crtTop){
	// 		console.log('end');
	// 		intervalID = setInterval(fadeOut, 50);
	// 	}else if(crtTop < targetPoint){
	// 		intervalID = setInterval(fadeIn, 50);
	// 	}
	// });
